/**
 * @file ESP32_MPU6050.cpp
 * @brief Implementation of the ESP32 MPU6050 library for IMU sensor communication and data processing.
...
bool ESP32_MPU6050::begin(GyroRange gyroRange, AccelRange accelRange, LpfBandwidth lpfBandwidth, uint32_t i2cClockSpeed, uint8_t expected_who_am_i)
{
  ...
  if (!writeRegister(MPU6050_USER_CTRL, 0))
...
uint8_t ESP32_MPU6050::dmpInitialize()
{
  ...
  if (!writeBit(MPU6050_PWR_MGMT_1, MPU6050_PWR1_DEVICE_RESET_BIT, 1))
    return 1;
  delay(DMP_RESET_DELAY_MS);
  if (!writeBits(MPU6050_USER_CTRL, MPU6050_USER_CTRL_FIFO_RESET_BIT, DMP_SIGNAL_PATH_RESET_BITS, DMP_SIGNAL_PATH_RESET_VALUE))
    return 1;
  delay(DMP_RESET_DELAY_MS);
  if (!writeRegister(MPU6050_PWR_MGMT_1, MPU6050_PWR1_CLKSEL_XGYRO))
    return 1;
  if (!writeRegister(MPU6050_RA_INT_ENABLE, DMP_INT_ENABLE_NONE))
    return 1;
  if (!writeRegister(MPU6050_FIFO_EN, DMP_FIFO_EN_NONE))
    return 1;
  if (!writeRegister(MPU6050_ACCEL_CONFIG, DMP_ACCEL_CONFIG_2G))
    return 1;
  ...
  if (!writeRegister(MPU6050_SMPLRT_DIV, DMP_SMPLRT_DIV_100HZ))
    return 1;
  if (!writeRegister(MPU6050_CONFIG, DMP_CONFIG_DLPF_188HZ))
    return 1;

  if (!writeProgMemoryBlock(dmpMemory, MPU6050_DMP_CODE_SIZE))
    return 1;

  if (!writeWord(MPU6050_RA_DMP_CFG_1, DMP_CFG_1_START_ADDRESS))
    return 1;
  if (!writeRegister(MPU6050_GYRO_CONFIG, DMP_GYRO_CONFIG_2000DPS))
    return 1;
  if (!writeRegister(MPU6050_USER_CTRL, DMP_USER_CTRL_FIFO_EN_RESET))
    return 1;
  if (!writeRegister(MPU6050_RA_INT_ENABLE, DMP_INT_ENABLE_RAW_DMP_INT_EN))
    return 1;
  if (!writeBit(MPU6050_USER_CTRL, MPU6050_USER_CTRL_FIFO_RESET_BIT, 1))
    return 1;
...
uint8_t ESP32_MPU6050::dmpGetCurrentFIFOPacket(uint8_t *data)
{ // overflow proof
...
uint8_t ESP32_MPU6050::dmpGetGravity(int16_t *data, const uint8_t *packet)
{
  ...
  data[2] = ((int32_t)qI[0] * qI[0] - (int32_t)qI[1] * qI[1] - (int32_t)qI[2] * qI[2] + (int32_t)qI[3] * qI[3]) / (2 * DMP_QUATERNION_SCALE);
  return status;
}
