...
*   FIFO buffer management (configuration, reading samples, overflow detection, packet size query).
...
void setup() {
  Serial.begin(115200);
    ...
    // You can specify these parameters in the begin() method. For example:
    // if (!sensor.begin(GYRO_RANGE_500DPS, ACCEL_RANGE_8G, LPF_42HZ_N_5MS, 400000, 0x68)) {
    if (!sensor.begin()) {
...
The MPU6050 features a FIFO (First-In, First-Out) buffer that allows for efficient batch reading of sensor data. This library provides comprehensive control over the FIFO.

**Configuring the FIFO:**
You can configure which sensor data (accelerometer, gyroscope, temperature) is stored in the FIFO using the `configureFifo()` method and a `FifoConfig` struct.

```cpp
FifoConfig myFifoConfig = {
    .enable_accel = true,
    .enable_gyro_x = true,
    .enable_gyro_y = true,
    .enable_gyro_z = true,
...
if (!sensor.configureFifo(myFifoConfig)) {
    Serial.println("Failed to configure FIFO!");
}
```

**Reading Data from FIFO:**
*   `getFifoCount()`: Returns the number of bytes currently in the FIFO buffer.
*   `getSensorFifoPacketSize()`: Returns the size (in bytes) of a single configured FIFO data packet.
*   `getFifoSample(SensorReadings &sample)`: Reads a single complete data packet from the FIFO and populates a `SensorReadings` struct. Returns `true` on success.
*   `getFifoSamples(SensorReadings samples[], uint16_t max_samples, uint16_t &actual_samples_read)`: Reads multiple samples into an array.

**FIFO Overflow Detection:**
*   `isFifoOverflowed()`: Checks if the FIFO buffer has overflowed.

**Resetting the FIFO:**
*   `resetFifo()`: Clears the FIFO buffer. This is automatically called by `configureFifo()`.

**Important Note on DMP and FIFO:**
The MPU6050's Digital Motion Processor (DMP) uses its own internal FIFO. Therefore, **you cannot configure or use the sensor FIFO while the DMP is enabled.** The `configureFifo()` method will return `false` if the DMP is active. Ensure the DMP is disabled before attempting to configure the sensor FIFO.
